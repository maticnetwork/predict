...
library Trade {
    ...
    function getSharesAvailable(IMarket _market, Order.Types _orderType, uint256 _outcome, uint256 _amount, address _creator) private view returns (uint256) {
        ...
        IAccountManager accounts = _market.getAccountManager();
        if (_orderType == Order.Types.Bid) {
            for (uint256 _i = 0; _i < _numberOfOutcomes; _i++) {
                if (_i != _outcome) {
                    // uint256 _creatorShareTokenBalance = _market.getShareToken(_i).balanceOf(_creator);
                    uint256 _creatorShareTokenBalance = accounts.balanceOf(_creator, address(_market), _i);
            ...
            _attosharesHeld = accounts.balanceOf(_creator, address(_market), _outcome);
        }
    ...
    function tradeMakerSharesForFillerShares(Data memory _data) internal returns (uint256, uint256) {
        ...
        (_marketCreatorFees, _reporterFees) = _data.contracts.accounts.jointSellCompleteSets(_data.contracts.market, _numberOfCompleteSets, _data.shortFundsAccount, _data.longFundsAccount, _data.order.outcome, getLongShareSellerDestination(_data), getShortShareSellerDestination(_data), _data.order.sharePriceLong, _data.affiliateAddress);
    ...
    function tradeMakerSharesForFillerTokens(Data memory _data) internal returns (bool) {
        ...
        if (_data.creator.direction == Direction.Short) {
            ...
            _data.contracts.accounts.trustedFillOrderTransfer(_data.shortFundsAccount, _data.filler.participantAddress, address(_data.contracts.market), _data.contracts.longShareToken, _numberOfSharesToTrade);
        } else {
            for (uint256 _i = 0; _i < _data.contracts.shortShareTokens.length; ++_i) {
                ...
                _data.contracts.accounts.trustedFillOrderTransfer(_data.longFundsAccount, _data.filler.participantAddress, address(_data.contracts.market), _data.contracts.shortShareTokens[_i], _numberOfSharesToTrade);
            }
        }
        ...
        _data.contracts.accounts.trustedTransfer(_data.filler.participantAddress, _data.creator.participantAddress, _tokensToCover);
    ...
    function tradeMakerTokensForFillerShares(Data memory _data) internal returns (bool) {
        ...
        if (_data.filler.direction == Direction.Short) {
            ...
            _data.contracts.accounts.trustedFillOrderTransfer(_data.filler.participantAddress, _data.creator.participantAddress, address(_data.contracts.market), _data.contracts.longShareToken, _numberOfSharesToTrade);
        } else {
            for (uint256 _i = 0; _i < _data.contracts.shortShareTokens.length; ++_i) {
                ...
                _data.contracts.accounts.trustedFillOrderTransfer(_data.filler.participantAddress, _data.creator.participantAddress, address(_data.contracts.market), _data.contracts.shortShareTokens[_i], _numberOfSharesToTrade);
            }
        }
        ...
        if (_data.order.orderId == bytes32(0)) {
            ...
            _data.contracts.accounts.trustedTransfer(_data.creator.participantAddress, _data.filler.participantAddress, _tokensToCover);
        } else {
            ...
            revert("FillOrder: Only supporting signed orders for now");
        }
    ...
    function tradeMakerTokensForFillerTokens(Data memory _data) internal returns (uint256) {
        ...
        // @TODO atvanguard Figure out this part
        ...
        _data.contracts.accounts.jointBuyCompleteSets(_data.contracts.market, _numberOfCompleteSets, _data.longFundsAccount, _data.shortFundsAccount, _data.order.outcome, _longRecipient, _shortRecipient, _data.order.sharePriceLong);
    ...
    function getFiller(Contracts memory _contracts, Order.Types _orderOrderType, address _address, uint256 _size) private view returns (Participant memory) {
        ...
        _sharesToSell = getFillerSharesToSell(_contracts, _contracts.longShareToken, _contracts.shortShareTokens, _address, _direction, _size);
    ...
    /*
     * @dev Modify getShortShareTokens to return an array of outcomes instead of their corresponding tokens
     */
    function getShortShareTokens(IMarket _market, uint256 _longOutcome) private view returns (uint256[] memory) {
        // IShareToken[] memory _shortShareTokens = new IShareToken[](_market.getNumberOfOutcomes() - 1);
        uint256[] memory _shortShareTokens = new uint256[](_market.getNumberOfOutcomes() - 1);
        for (uint256 _outcome = 0; _outcome < _shortShareTokens.length + 1; ++_outcome) {
            ...
            _shortShareTokens[_index] = _outcome;
    ...
    function getFillerSharesToSell(Contracts memory _contracts, uint256 _longShareToken, uint256[] memory _shortShareTokens, address _filler, Direction _fillerDirection, uint256 _fillerSize) private view returns (uint256) {
        uint256 _sharesAvailable = SafeMathUint256.getUint256Max();
        // Plug-in accountManager to query balances on Matic
        IAccountManager accounts = _contracts.accounts;
        if (_fillerDirection == Direction.Short) {
            // _sharesAvailable = _longShareToken.balanceOf(_filler);
            _sharesAvailable = accounts.balanceOf(_filler, address(_contracts.market), _longShareToken);
        } else {
            for (uint256 _outcome = 0; _outcome < _shortShareTokens.length; ++_outcome) {
                ...
                _sharesAvailable = accounts.balanceOf(_filler, address(_contracts.market), _outcome).min(_sharesAvailable);
            }
        }
...
contract FillOrder /* is Initializable, ReentrancyGuard, IFillOrder */ {
    ...
    function fillOrderInternal(address _filler, Trade.Data memory _tradeData, uint256 _amountFillerWants, bytes32 _tradeGroupId) internal /* nonReentrant */ returns (uint256) {
        ...
        // Removing this since we are only supporting signed orders for now, this condition has already been asserted on in tradeMakerTokensForFillerShares
        ...
        // @TODO atvanguard Dig into and handle this part later
    ...
    function sellCompleteSets(Trade.Data memory _tradeData) internal returns (bool) {
        ...
        IAccountManager _accounts = _tradeData.contracts.accounts;

        // uint256 _fillerCompleteSets = _market.getShareToken(0).balanceOf(_filler);
        uint256 _fillerCompleteSets = _accounts.balanceOf(_filler, address(_market), 0);
        // uint256 _creatorCompleteSets = _market.getShareToken(0).balanceOf(_creator);
        uint256 _creatorCompleteSets = _accounts.balanceOf(_creator, address(_market), 0);
        ...
        for (uint256 _outcome = 1; _outcome < _market.getNumberOfOutcomes(); ++_outcome) {
            ...
            _creatorCompleteSets = _creatorCompleteSets.min(_accounts.balanceOf(_creator, address(_market), _outcome));
            // _fillerCompleteSets = _fillerCompleteSets.min(_shareToken.balanceOf(_filler));
            _fillerCompleteSets = _fillerCompleteSets.min(_accounts.balanceOf(_filler, address(_market), _outcome));
        }
        ...
        if (_fillerCompleteSets > 0) {
            ...
            _accounts.sellCompleteSets(_filler, _market, _fillerCompleteSets, _tradeData.affiliateAddress);
        }
        ...
        if (_creatorCompleteSets > 0) {
            ...
            _accounts.sellCompleteSets(_creator, _market, _creatorCompleteSets, _tradeData.affiliateAddress);
        }
